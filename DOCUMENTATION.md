...
* **Verify Fix Mechanism**: When an issue is physically resolved (e.g., a pothole is filled), the community can submit a "Verify Fix". Once an issue receives 5 independent fix verifications from the community, the issue is officially closed and the original reporter finally receives their Pending BP Points as a delayed reward for their initial contribution.
